...
const cardContainer = document.querySelector(".tudo");

todosPlanetas.forEach(planeta => {
    const planetaCard = document.createElement("div");
    planetaCard.classList.add("planet");
    
    planetaCard.innerHTML = `
        <img src="${planeta.imagem}" alt="${planeta.nome}">
        <div class="desenvolvimento--planet">
            <h2 class="titulo--planet">${planeta.nome}</h2>
            <p>${planeta.sobre}</p>
        </div>
    `;
    
    cardContainer.appendChild(planetaCard);
});